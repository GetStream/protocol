...
call.create(
		data=CallRequest(
			starts_at=2024-05-31T12:05:22+00:00),
...
A call can enter and leave backstage mode multiple times. If you use `join_ahead_time_seconds`, you might need to update the `starts_at` field as well. Otherwise, `stopLive` will not prevent regular users from joining the backstage.

The stopLive endpoint allows you to update the `starts_at` call property as well.
...
// call stopLive() with startsAt update
call.stopLive({startsAt: '2024-05-31T12:05:22+00:00'});
```
...
call.stop_live(starts_at=2024-05-31T12:05:22+00:00)
```
