...
client = Stream(api_key="your_api_key", api_secret="your_api_secret")
response = client.video.get_or_create_call(call_type=calltype_name, call_id=call_id, data=call_request_data)
```
...
```py
client.video.update_members(
    call_type="your_call_type",
    call_id="your_call_id",
    data={
        "update_members": [
            {"user_id": "sara"},
...
# to stop recording
client.video.stop_broadcasting(call_id=call_id, call_type=call_type)
```
...
response = client.video.get_or_create_call(call_type=call_type, call_id=call_id, data=data)
rtmp_address = response["call"]["ingress"]["rtmp"]["address"]
address = f"{rtmp_address}/{call_type}/{call_id}/{VIDEO_API_KEY}/{client.create_token()}"
```
...
```py
recordings = client.video.list_recordings(
    call_type="your_call_type",
    call_id="your_call_id",
