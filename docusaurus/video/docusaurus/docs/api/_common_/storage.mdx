...
2. (Optional) Check storage configuration for correctness.
    Calling check endpoint will create a test markdown file in the storage to verify the configuration. It will return an error if the file is not created.  3
...
# 1a. (Optional) Check storage configuration for correctness
