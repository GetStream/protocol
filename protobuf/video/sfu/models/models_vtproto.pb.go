...
func (m *TrackInfo) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	...
	if m.AudioBitrateType != 0 {
		i = encodeVarint(dAtA, i, uint64(m.AudioBitrateType))
		i--
		dAtA[i] = 0x68
	}
...
func (m *TrackInfo) SizeVT() (n int) {
	...
	if m.AudioBitrateType != 0 {
		n += 1 + sov(uint64(m.AudioBitrateType))
	}
	if m.unknownFields != nil {
		n += len(m.unknownFields)
	}
	...
	for iNdEx < l {
		...
		case 13:
			...
			for shift := uint(0); ; shift += 7 {
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.AudioBitrateType |= AudioBitrateType(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
