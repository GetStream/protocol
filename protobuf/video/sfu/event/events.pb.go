...
//
// When this message is received, the client (say, React component) could gray
// out the audio, video or screenshare buttons if the current grants for them
// is false or re-enable them if they are granted. If there is no change then
// they don't have to do anything.
//
// Let us say, the first message has grants {audio:true, video:true}, the client
// component would gray out screenshare button as it is not allowed. Suppose, the
// next update has grants {audio: true} (video and screenshare are not present
// in the message), then the client component would gray out video and screenshare
// along with the mute icon. SFU will mute the respective tracks if they are being
// published at the time of permission update. Nevertheless, the clients should
// stop publishing the tracks which are not permitted anymore. In the previous
// example, the video should be stopped and the button to turn on the video is
// disabled along with "muted video" icon.
...
var file_video_sfu_event_events_proto_rawDesc = []byte{
	...
	0x22, 0x5f, 0x0a, 0x11, 0x43, 0x61, 0x6c, 0x6c, 0x47, 0x72, 0x61, 0x6e, 0x74, 0x73, 0x55, 0x70,
	...
	0x73, 0x42, 0x5d, 0x42, 0x0b, 0x53, 0x66, 0x75, 0x45, 0x76, 0x65, 0x6e, 0x74, 0x73, 0x56, 0x31,
